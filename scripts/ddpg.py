...
def shutdown_gazebo():
    try:
        ...
        print("Gazebo shutdown successful.")
    except Exception as e:
        print("Error shutting down Gazebo:", e)
...
class DroneNavigationEnv(gym.Env):
    ...
    def callback(self, ros_data):
        ...
        centers = []
        # loop through all contours and calculate the center
        for c in cnts:
    ...
    def take_action(self, action):
        ...
        linear_x = 5*(action[0]+1)/2 #scaling (-1,1) to (0,1) because velocity should be positive
        linear_y = action[1]*5
        z_vel = action[2]*150
...
def main():
    ...
    #     )
    model = sb3.PPO('MlpPolicy', env, verbose=1, tensorboard_log="./board/")
    ...
    #     print(f"Episode {timestep+1/total_timesteps_}")
    #     action, _states = model.predict(env.get_observations())  # Predict action based on the updated observation
    ...
    model.save("/home/ecem/Documents/drone_ws/ddpg")

    # model = sb3.PPO.load("/home/ecem/Documents/drone_ws/ddpg")
    # obs = env.reset()
