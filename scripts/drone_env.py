...
from target_points import check_balls_in_environment
...
class DroneEnv(Env):
    def __init__(self):
        ...
        # self.min_non_zero_distance = 100
        ...
        #                         high=np.array([1.0, 1.0]), shape=(2,),
        #                         dtype=np.float32)  
    
        self.action_space = gym.spaces.Box(low=-1, high=1, shape=(2,), dtype=np.float32)
        self.observation_space = gym.spaces.Box(low=0, high=255, shape=(60,80,1), dtype=np.uint8)
        # self.observation_space = gym.spaces.Box(low=-10, high=10, shape=(6,), dtype=float)


        # self.observation_space = gym.spaces.Dict({
        #     "depth_map": Box(low=0, high=255,shape=(60,80,1), dtype=np.uint8),
        #     "current_pose": Box(low=-10, high=10, shape=(6,), dtype=float)})
        
       
        # Publishers, subscribers and services
        self.current_pose_sub = rospy.Subscriber("pose", Position, self.position_callback)    
        self.image_sub = rospy.Subscriber("/camera/depth/image_raw", Image, self.camera_callback) # depth camera
