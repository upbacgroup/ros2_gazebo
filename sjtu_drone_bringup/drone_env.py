...
class DroneEnv(gym.Env):
    def __init__(self):
        ...
        # self.observation_space = gym.spaces.Box(low=0, high=255, shape=(2,), dtype=np.float32)
    ...
    def step(self, action):
        ...
        reward = -1 + 2/(1+np.exp(0.5*self.min_distance/(100*self.num_green_pixels+1))) - collision_penalty
        # reward = -1 + 2/(1+np.exp(0.5*self.min_distance)) - collision_penalty
